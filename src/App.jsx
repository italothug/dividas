...
export default function App() {
  ...
  const [expense,setExpense] = useState({desc:'',valor:'',tipo:'fixa',parcelas:1})
  ...
  const totalIn=data.entradas.reduce((sum,item)=>sum+item.valor,0)
  ...
  return <main className="page"><div className="shell">
    ...
    <section className="book">
      ...
      <Section title="Entradas" icon={<ArrowUpCircle/>}>{data.entradas.length===0&&<Empty text="Nenhuma entrada neste mês ainda."/>}{data.entradas.map(item=><Row key={item.id} item={item} types={IN_TYPES} onDelete={()=>updateMonth(m=>({...m,entradas:m.entradas.filter(i=>i.id!==item.id)}))}/>)}<EntryForm value={income} setValue={setIncome} onAdd={addIncome}/></Section>
      <hr/>
      <Section title="Saídas" icon={<ArrowDownCircle/>}>{data.saidas.length===0&&<Empty text="Nenhuma saída neste mês ainda."/>}{data.saidas.map(item=><Row key={item.id} item={item} types={OUT_TYPES} expense onToggle={()=>updateMonth(m=>({...m,saidas:m.saidas.map(i=>i.id===item.id?{...i,paga:!i.paga}:i)}))} onDelete={()=>updateMonth(m=>({...m,saidas:m.saidas.filter(i=>i.id!==item.id)}))}/>)}<ExpenseForm value={expense} setValue={setExpense} onAdd={addExpense}/></Section>
    </section>
  </div></main>
}
...
function Section({title,icon,children}){return <section className="section"><h2>{icon}{title}</h2><div className="rows">{children}</div></section>}
function Empty({text}){return <p className="empty">{text}</p>}
function Row({item,types,expense,onToggle,onDelete}){const type=types[item.tipo]||['Outro','neutral'];return <div className={`row ${item.paga?'paid':''}`}>{expense&&<button className="check" onClick={onToggle}>{item.paga?'✓':''}</button>}<span className="description">{item.desc}</span>{item.parcelaTotal>1&&<span className="installment">{item.parcelaAtual}/{item.parcelaTotal}</span>}<span className={`pill ${type[1]}`}>{type[0]}</span><strong className={expense?'out':'in'}>{money(item.valor)}</strong><button className="trash" onClick={onDelete}><Trash2/></button></div>}
...
function ExpenseForm({value,setValue,onAdd}){return <div className="form"><input placeholder="Descrição (ex: aluguel)" value={value.desc} onChange={e=>setValue({...value,desc:e.target.value})}/><input placeholder="Valor" inputMode="decimal" value={value.valor} onChange={e=>setValue({...value,valor:e.target.value})}/><select value={value.tipo} onChange={e=>setValue({...value,tipo:e.target.value})}>{Object.entries(OUT_TYPES).map(([k,v])=><option key={k} value={k}>{v[0]}</option>)}</select>{value.tipo!=='fixa'&&<input className="parcelas" placeholder="Nº parcelas" inputMode="numeric" value={value.parcelas} onChange={e=>setValue({...value,parcelas:e.target.value})}/>}<button className="add red" onClick={onAdd}><Plus/> adicionar</button></div>}
