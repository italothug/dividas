...
export default function App() {
  ...
  useEffect(() => {
    ...
    start()
    const listener=isSupabaseConfigured ? supabase.auth.onAuthStateChange((event,newSession)=>{ setSession(newSession); if(event==='PASSWORD_RECOVERY')setRecoveryOpen(true); if(newSession?.user) loadCloudState(newSession.user.id).then(cloud=>{versionRef.current=cloud?.version??null;setState(cloud?.state || loadLocalState() || initialState())}).catch(()=>{}) }).data.subscription : null
