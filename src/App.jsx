...
import { ArrowDownCircle, ArrowUpCircle, Download, History, LogIn, LogOut, Pencil, Plus, RefreshCw, ShieldCheck, Smartphone, Trash2, Upload, Wallet, WifiOff, X } from 'lucide-react'
import { isSupabaseConfigured, supabase } from './lib/supabase'
import { approveAccount, clearLocalState, clearPendingSync, loadAccountAccess, loadCloudHistory, loadCloudState, loadLocalState, loadPendingAccounts, loadPendingSync, saveCloudState, saveLocalState, savePendingSync } from './lib/storage'
import { cardDetailSummary, financialSummary, parseCurrency, validateLedgerState } from './lib/ledger'

const MONTHS = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']
const OUT_TYPES = { fixa:['Fixa','green'], investimento:['Investimento','blue'], cartao:['Cartão','gold'], divida:['Dívida','red'] }
const IN_TYPES = { salario:['Salário','green'], extra:['Renda extra','blue'], devem:['Devedor','purple'] }
const IN_ORDER = { salario:0, extra:1, devem:2 }
const OUT_ORDER = { fixa:0, investimento:1, cartao:2, divida:3 }
const sortEntries = items => [...items].sort((a,b)=>(IN_ORDER[a.tipo]??99)-(IN_ORDER[b.tipo]??99)||b.valor-a.valor)
const sortExpenses = items => [...items].sort((a,b)=>(OUT_ORDER[a.tipo]??99)-(OUT_ORDER[b.tipo]??99)||b.valor-a.valor)
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      if (!active) return
      setSession(current)
      const access=current?.user?await loadAccountAccess(current.user.id).catch(()=>null):null
      setAccountAccess(access)
      const local=loadLocalState()
      let loaded=local
      if (current?.user&&access?.approved) {
    ...
    start()
    const listener=isSupabaseConfigured ? supabase.auth.onAuthStateChange((event,newSession)=>{ setSession(newSession); if(event==='PASSWORD_RECOVERY')setRecoveryOpen(true);if(event==='SIGNED_OUT'){clearLocalState();versionRef.current=null;setState(initialState());setStatus('Dados da conta removidos deste dispositivo.')} if(!newSession){setAccountAccess(null);return}loadAccountAccess(newSession.user.id).then(access=>{setAccountAccess(access);if(access.approved)loadCloudState(newSession.user.id).then(cloud=>{versionRef.current=cloud?.version??null;setState(cloud?.state || loadLocalState() || initialState())}).catch(()=>{})}).catch(()=>setAccountAccess(null)) }).data.subscription : null
  ...
  async function logout(){clearLocalState();clearPendingSync();versionRef.current=null;setState(initialState());setAccountAccess(null);setStatus('Dados deste dispositivo removidos.');await supabase.auth.signOut()}
  async function openAdmin(){setStatus('Carregando solicitações...');try{setPendingAccounts(await loadPendingAccounts());setAdminOpen(true);setStatus('')}catch{setStatus('Não foi possível carregar as solicitações.')}}
  async function approve(userId){try{await approveAccount(userId);setPendingAccounts(items=>items.filter(item=>item.user_id!==userId));setStatus('Conta aprovada com sucesso.')}catch{setStatus('Não foi possível aprovar esta conta.')}}
  if(session&&!accountAccess)return <main className="loading">Verificando autorização da conta...</main>
  if(session&&!accountAccess?.approved)return <PendingApproval email={session.user.email} onLogout={logout}/>
  ...
  return <main className="page"><div className="shell">
    ...
    <header><div className="brand"><Wallet aria-hidden="true"/><div><h1>Caderno de Contas</h1><p>{longLabel(key)}</p></div></div><div className="header-actions"><label className="sr-only" htmlFor="year-select">Ano</label><select id="year-select" value={selectedYear} onChange={e=>{const y=Number(e.target.value);setYear(y);const first=state.mesesLista.find(m=>Number(m.slice(0,4))===y);if(first)persist({...state,mesAtual:first})}}>{years.map(y=><option key={y}>{y}</option>)}</select>{accountAccess?.is_admin&&<button type="button" className="auth-button" onClick={openAdmin}><ShieldCheck aria-hidden="true"/> Aprovações</button>}<button type="button" className="auth-button" onClick={()=>session?logout():setAuthOpen(true)}>{session?<><LogOut aria-hidden="true"/> Sair</>:<><LogIn aria-hidden="true"/> Entrar</>}</button></div></header>
    {authOpen&&<AuthDialog onClose={()=>setAuthOpen(false)}/>} 
    {recoveryOpen&&<PasswordDialog onClose={()=>setRecoveryOpen(false)}/>} 
    ...
    {adminOpen&&<AdminDialog items={pendingAccounts} onApprove={approve} onClose={()=>setAdminOpen(false)}/>}
    ...
    <section className="book" id="conteudo">
      ...
      <Section title="Entradas" tone="income" icon={<ArrowUpCircle/>}>{data.entradas.length===0&&<Empty text="Nenhuma entrada neste mês ainda."/>}{sortEntries(data.entradas).map(item=><Row key={item.id} item={item} types={IN_TYPES} onToggle={()=>updateMonth(m=>({...m,entradas:m.entradas.map(i=>i.id===item.id?{...i,recebida:i.recebida===false}:i)}))} onEdit={()=>setEditing({kind:'entradas',item})} onDelete={()=>deleteItem('entradas',item)}/>)}<EntryForm value={income} setValue={setIncome} onAdd={addIncome}/></Section>
      <hr/>
      <Section title="Saídas" tone="expense" icon={<ArrowDownCircle/>}>{data.saidas.length===0&&<Empty text="Nenhuma saída neste mês ainda."/>}{sortExpenses(data.saidas).map(item=><Row key={item.id} item={item} types={OUT_TYPES} expense onToggle={()=>updateMonth(m=>({...m,saidas:m.saidas.map(i=>i.id===item.id?{...i,paga:!i.paga}:i)}))} onEdit={()=>setEditing({kind:'saidas',item})} onDelete={()=>deleteItem('saidas',item)}/>)}<ExpenseForm value={expense} setValue={setExpense} onAdd={addExpense}/></Section>
    </section>
  </div></main>
}
...
function PendingApproval({email,onLogout}){return <main className="approval-page"><div className="approval-card"><ShieldCheck aria-hidden="true"/><h1>Conta aguardando aprovação</h1><p>O e-mail <strong>{email}</strong> foi confirmado. Um administrador precisa liberar o acesso ao caderno.</p><button type="button" className="primary" onClick={onLogout}><LogOut aria-hidden="true"/> Sair</button></div></main>}
function AdminDialog({items,onApprove,onClose}){return <DialogShell titleId="admin-title" onClose={onClose} className="admin-dialog"><button type="button" className="close" aria-label="Fechar" onClick={onClose}><X aria-hidden="true"/></button><h2 id="admin-title">Aprovar contas</h2><p>Libere somente pessoas que você reconhece.</p>{items.length===0?<p className="empty">Nenhuma solicitação pendente.</p>:<div className="approval-list">{items.map(item=><div key={item.user_id}><span>{item.email}<small>{new Date(item.created_at).toLocaleString('pt-BR')}</small></span><button type="button" onClick={()=>onApprove(item.user_id)}>Aprovar</button></div>)}</div>}</DialogShell>}
function authError(error){const text=error?.message||'Não foi possível concluir. Tente novamente.';if(/invalid login credentials/i.test(text))return'E-mail ou senha incorretos.';if(/email not confirmed/i.test(text))return'Confirme seu e-mail antes de entrar.';if(/password should be at least/i.test(text))return'A senha precisa ter pelo menos 6 caracteres.';if(/user already registered/i.test(text))return'Este e-mail já possui uma conta.';if(/rate limit|security purposes/i.test(text))return'Aguarde um pouco antes de tentar novamente.';return text}
function AuthDialog({onClose}){
  const [mode,setMode]=useState('login'); const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); const [confirm,setConfirm]=useState(''); const [awaiting,setAwaiting]=useState(false); const [message,setMessage]=useState('')
  function changeMode(next){setMode(next);setAwaiting(false);setPassword('');setConfirm('');setMessage('')}
  async function login(){if(!email||!password){setMessage('Informe o e-mail e a senha.');return}setMessage('Entrando...');const{error}=await supabase.auth.signInWithPassword({email,password});if(error){setMessage(authError(error));return}onClose()}
  async function signup(){if(!email||password.length<6){setMessage('Use uma senha com pelo menos 6 caracteres.');return}if(password!==confirm){setMessage('As senhas não são iguais.');return}setMessage('Criando conta...');const{data,error}=await supabase.auth.signUp({email,password,options:{emailRedirectTo:window.location.origin}});if(error){setMessage(authError(error));return}if(data.session){onClose();return}setAwaiting(true);setMessage('Confirme o e-mail e clique em “Já confirmei”. Depois disso, a conta aguardará aprovação do administrador.')}
  async function magic(){if(!email){setMessage('Informe o seu e-mail.');return}setMessage('Enviando...');const{error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin}});setMessage(error?authError(error):'Link enviado. Abra-o no navegador em que deseja entrar.')}
  async function reset(){if(!email){setMessage('Informe o seu e-mail.');return}setMessage('Enviando...');const{error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin});setMessage(error?authError(error):'Enviamos um link para você definir uma nova senha.')}
  const submit=mode==='login'?login:mode==='signup'?(awaiting?login:signup):magic
  return <DialogShell titleId="auth-title" onClose={onClose}><button type="button" className="close" aria-label="Fechar" onClick={onClose}><X aria-hidden="true"/></button><h2 id="auth-title">Sincronize seu caderno</h2><div className="auth-tabs" role="tablist" aria-label="Forma de acesso"><button type="button" className={mode==='login'?'active':''} onClick={()=>changeMode('login')}>Entrar</button><button type="button" className={mode==='signup'?'active':''} onClick={()=>changeMode('signup')}>Criar conta</button><button type="button" className={mode==='magic'?'active':''} onClick={()=>changeMode('magic')}>Link por e-mail</button></div><label>E-mail<input type="email" autoComplete="email" placeholder="voce@exemplo.com" value={email} disabled={awaiting} onChange={e=>setEmail(e.target.value)}/></label>{mode!=='magic'&&<label>Senha<input type="password" autoComplete={mode==='login'?'current-password':'new-password'} minLength="6" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}/></label>}{mode==='signup'&&!awaiting&&<label>Confirmar senha<input type="password" autoComplete="new-password" minLength="6" value={confirm} onChange={e=>setConfirm(e.target.value)} onKeyDown={e=>e.key==='Enter'&&signup()}/></label>}<button type="button" className="primary" onClick={submit}>{mode==='login'?'Entrar':mode==='signup'?(awaiting?'Já confirmei':'Criar conta'):'Enviar link de acesso'}</button>{mode==='login'&&<button type="button" className="auth-secondary" onClick={reset}>Esqueci minha senha</button>}<small aria-live="polite">{message}</small></DialogShell>
}
function PasswordDialog({onClose}){const [password,setPassword]=useState('');const [confirm,setConfirm]=useState('');const [message,setMessage]=useState('');async function save(){if(password.length<6){setMessage('Use uma senha com pelo menos 6 caracteres.');return}if(password!==confirm){setMessage('As senhas não são iguais.');return}setMessage('Salvando...');const{error}=await supabase.auth.updateUser({password});if(error){setMessage(authError(error));return}onClose()}return <DialogShell titleId="password-title" onClose={onClose}><button type="button" className="close" aria-label="Fechar" onClick={onClose}><X aria-hidden="true"/></button><h2 id="password-title">Defina sua nova senha</h2><p>Depois de salvar, você poderá entrar com esta senha em qualquer navegador.</p><label>Nova senha<input type="password" autoComplete="new-password" minLength="6" value={password} onChange={e=>setPassword(e.target.value)}/></label><label>Confirmar senha<input type="password" autoComplete="new-password" minLength="6" value={confirm} onChange={e=>setConfirm(e.target.value)} onKeyDown={e=>e.key==='Enter'&&save()}/></label><button type="button" className="primary" onClick={save}>Salvar nova senha</button><small aria-live="polite">{message}</small></DialogShell>}
function EditDialog({edit,onSave,onClose}){const expense=edit.kind==='saidas';const [item,setItem]=useState({...edit.item});const [value,setValue]=useState(String(edit.item.valor).replace('.',','));const [detail,setDetail]=useState({desc:'',valor:''});const [error,setError]=useState('');const types=expense?OUT_TYPES:IN_TYPES;function addDetail(){const valor=parseCurrency(detail.valor);if(!detail.desc.trim()||!Number.isFinite(valor)||valor<=0)return;setItem({...item,detalhes:[...(item.detalhes||[]),{id:id(),desc:detail.desc.trim(),valor}]});setDetail({desc:'',valor:''})}function save(){const valor=parseCurrency(value);if(!item.desc.trim()||!Number.isFinite(valor)||valor<=0){setError('Informe uma descrição e um valor válido.');return}const next={...item,desc:item.desc.trim(),valor};if(expense&&next.tipo==='cartao'&&cardDetailSummary(valor,next.detalhes).exceeds){setError('Os subvalores ultrapassam o total do cartão.');return}onSave(next)}return <DialogShell titleId="edit-title" onClose={onClose} className="edit-dialog"><button type="button" className="close" aria-label="Fechar" onClick={onClose}><X aria-hidden="true"/></button><h2 id="edit-title">Editar lançamento</h2><label>Descrição<input value={item.desc} onChange={e=>setItem({...item,desc:e.target.value})}/></label><label>Valor<input inputMode="decimal" value={value} onChange={e=>setValue(e.target.value)}/></label><label>Categoria<select value={item.tipo} onChange={e=>setItem({...item,tipo:e.target.value,detalhes:e.target.value==='cartao'?item.detalhes:[]})}>{Object.entries(types).map(([key,type])=><option key={key} value={key}>{type[0]}</option>)}</select></label>{expense&&!['fixa','investimento'].includes(item.tipo)&&<div className="edit-grid"><label>Parcela atual<input type="number" min="1" value={item.parcelaAtual||1} onChange={e=>setItem({...item,parcelaAtual:Math.max(1,Number(e.target.value))})}/></label><label>Total de parcelas<input type="number" min="1" value={item.parcelaTotal||1} onChange={e=>setItem({...item,parcelaTotal:Math.max(1,Number(e.target.value))})}/></label></div>}{expense&&item.tipo==='cartao'&&<div className="edit-details"><strong>Detalhes do cartão</strong>{item.detalhes?.map(entry=><div className="detail-preview" key={entry.id}><span>{entry.desc}</span><strong>{money(entry.valor)}</strong><button type="button" aria-label={`Remover ${entry.desc}`} onClick={()=>setItem({...item,detalhes:item.detalhes.filter(d=>d.id!==entry.id)})}><X aria-hidden="true"/></button></div>)}<div className="detail-fields"><input aria-label="Nova subdescrição" placeholder="Subdescrição" value={detail.desc} onChange={e=>setDetail({...detail,desc:e.target.value})}/><input aria-label="Novo subvalor" placeholder="Subvalor" inputMode="decimal" value={detail.valor} onChange={e=>setDetail({...detail,valor:e.target.value})}/><button type="button" className="detail-add" onClick={addDetail}><Plus aria-hidden="true"/> incluir</button></div></div>}<small className="form-error" aria-live="polite">{error}</small><button type="button" className="primary" onClick={save}>Salvar alterações</button></DialogShell>}
